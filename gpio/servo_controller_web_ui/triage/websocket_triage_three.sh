...
check_http_server() {
    ...
    if netstat -tuln | grep ':5000' > /dev/null; then
        ...
        # Manually restart Flask by running it as a background process
        nohup python3 /home/pi/servo_project/controller/app.py > /home/pi/servo_project/controller/webui/flask.log 2>&1 &

        # Check if Flask is running after restarting
        if netstat -tuln | grep ':5000' > /dev/null; then
            echo "HTTP server successfully restarted."
        else
            echo "Failed to restart HTTP server. Check Flask application logs for issues."
        fi
    fi
}
