...
def move_servo(channel, value):
    if not hold_state[channel] and not global_hold:  # Only move if hold is not active
...
def display_status():
    ...
    print(f"\rLX: {lx} [{servo_positions[0]:3}°]  LY: {ly} [{servo_positions[1]:3}°]  "
          f"RY: {ry} [{servo_positions[3]:3}°]  RX: {rx} [{servo_positions[2]:3}°]  "
          f" |  {lock_status}  \n"
          f" A: {hold_status[0]}       B: {hold_status[1]}       X: {hold_status[2]}       Y: {hold_status[3]}  ", end="")
        ...
        for event in gamepad.read_loop():
            if event.type == evdev.ecodes.EV_ABS:
                ...
                if event.code == evdev.ecodes.BTN_SOUTH and event.value == 1:  # A button → Hold Servo 0 (LX)
                    ...
                    display_status()
                elif event.code == evdev.ecodes.BTN_WEST and event.value == 1:  # X button → Hold Servo 2 (RX)
                    hold_state[2] = not hold_state[2]
                    display_status()
                elif event.code == evdev.ecodes.BTN_NORTH and event.value == 1:  # Y button → Hold Servo 3 (RY)
