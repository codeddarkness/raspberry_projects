...
servo_positions = {0: 375, 1: 375, 2: 375, 3: 375}  # Default to center position
...
def move_servo(channel, value):
    if not hold_state[channel]:  # Only move if hold is not active
        ...
        servo_positions[channel] = pwm_value  # Store position
        display_status()
...
# Display status of joysticks and hold buttons
def display_status():
    arrows = {
        "left": "←", "right": "→",
        "up": "↑", "down": "↓",
    ...
    lx = arrows["left"] if servo_positions[0] < 375 else arrows["right"] if servo_positions[0] > 375 else arrows["neutral"]
    ly = arrows["up"] if servo_positions[1] < 375 else arrows["down"] if servo_positions[1] > 375 else arrows["neutral"]
    ry = arrows["up"] if servo_positions[2] < 375 else arrows["down"] if servo_positions[2] > 375 else arrows["neutral"]
    rx = arrows["left"] if servo_positions[3] < 375 else arrows["right"] if servo_positions[3] > 375 else arrows["neutral"]

    hold_status = {ch: "ON" if hold_state[ch] else "OFF" for ch in hold_state}

    print(f"\rLX: {lx}  LY: {ly}  RY: {ry}  RX: {rx} | Hold - B: {hold_status[0]} A: {hold_status[1]} Y: {hold_status[2]} X: {hold_status[3]} ", end="")
